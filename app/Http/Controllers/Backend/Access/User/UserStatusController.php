...
class UserStatusController extends Controller
{
    ...
    public function mark(User $user, $status, EditUserRequest $request)
    {
        ...
        return redirect()->route($status == 1 ? 'admincp.access.user.index' : 'admin.access.user.deactivated')->withFlashSuccess(trans('alerts.backend.users.updated'));
    }
    ...
    public function delete(User $deletedUser, DeleteUserRequest $request)
    {
        ...
        return redirect()->route('admincp.access.user.deleted')->withFlashSuccess(trans('alerts.backend.users.deleted_permanently'));
    }
    ...
    public function restore(User $deletedUser, DeleteUserRequest $request)
    {
        ...
        return redirect()->route('admincp.access.user.index')->withFlashSuccess(trans('alerts.backend.users.restored'));
    }
}
